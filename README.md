...
### 20.1   （138） 分布式事务问题由来
    ...
    2.分布式后：
        ...
        2.多应用多库 N V N 【分布式事务】
        3.示例：
            用户购买商品的业务逻辑：由三个微服务支撑
                ·仓储服务-对给定的商品扣除仓储数量
                ·订单服务-根据采购需求创建订单
                ·账户服务-从用户账户中扣除余额
            单体应用被拆分为微服务应用，原来三个模块被拆分为独立的三个应用，分别使用三个独立的数据源
            此时每个单体应用的事务由本地事务保证，但是全局数据的一致性无法保证
    3.分布式问题总结
        一次业务操作需要跨多个应用进行远程调用，就会产生分布式事务问题
...
### 20.2   （139） Seata 术语
    ...
    2.一个典型的分布式事务过程（面试常问）
        1.分布式事务一致性ID+三组件模型
            1.Transaction ID XID：全局唯一得事务id
            2.三组件概念（均来自官网解释）
...
### 20.11  （148） Seata之原理简介
    ...
    1.seata:Simple Extensible Autonomous Transaction Architecture
        1.2019年1月 蚂蚁金服+阿里巴巴共同开源的分布式事务解决方案
        2.简单可拓展自治事务框架
        3.2020起始,参加工作用的1.0以后的版本:GA版本 v1.0.0-GA GA代表发布版，正式稳定版本
            因为0.9.0版本的不支持集群，小公司可以使用，大公司使用商用版本
    2.TC、TM、RM
        TC：事务全局协调者，即seata服务
        TM：事务管理者，事务发起方，即添加注解@GlobalTransactional所在的应用
        RM: 事务资源管理器，事务参与方，每个应用或者数据库都可作为RM 
    3.分布式事务执行流程
        1.TM开启分布式事务-TM向TC注册全局事务记录
        2.按业务场景编排数据库，服务等事务内资源（RM向TC汇报资源准备状态）
        3.TM结束分布式事务，事务一阶段结束，TM通知TC提交或回滚分布式事务
        4.TC汇总事务信息，决定分布式事务是提交还是回滚
        5.TC通知所有RM提交或者回滚资源，事务二阶段结束
    4.官网：http://seata.io/zh-cn/docs/overview/what-is-seata.html
        AT 对应 阿里云的GTS：spring-aop思想+反向补偿 
        见图
        如果出现脏写-需要转人工处理
        可以通过debug，查看那些系统表中的数据变化
...
### 21.1   （149） 雪花算法上
    ...
    1.集群高并发下如何保证分布式唯一全局id生成？
    2.为什么需要分布式id？ 分库分表导致的id冲突问题 或者自增主键不满足需求
        1.什么是分布式唯一ID？为什么要用全局唯一ID？
            某商城订单数据表，如果订单数据放到一张表内，那订单ID可以简单使用数据库表的自增id，可以保证订单id递增不会重复。
            但是，随着业务越来越庞大，单表数据量太大，会出现数据库瓶颈问题，这时往往要对订单表进行分库分表，多机房部署，
            可以按日期或取模的方式分表，这时就带来一个问题，分表后每张表自增id是独立的，肯定会生成重复的ID，
            因此，就不能使用数据库自增id来作为订单ID，必须使用全局唯一ID，每个订单id在全局是唯一的。
        2.唯一ID需要满足什么特性？
            全局唯一ID需要满足3个特性：
            - 全局唯一性：不能出现重复的ID号，
            ...
            - 占用空间尽可能小，一般使用64bit
    3.id生成规则的硬性要求
        1.全局唯一：整个系统唯一，有且只有一个
        2.趋势递增：innodb引擎使用聚集索引，使用Btree来存储索引数据，主键选择上应该尽量使用有序的主键保证写入性能
        3.单调递增：尽量保证下一个id一定大于上一个id，例如事务版本号，IM增量消息，排序等特殊需求
        4.信息安全：如果id是连续的，恶意用户就很容易按照顺序下载指定url，如果是订单号就很危险了，竞争对手就可以知道一天的单量。所以在一些引用应用场景下
            就需要ID无规则，不规则，让竞争对手无法猜测
        5.含时间戳：能够在开发中快速了解分布式id的生成时间
    4.ID生成系统的可用性要求
        1.高可用-发一个获取分布式id的请求，服务器就要保证99.999%的情况下创建一个唯一的分布式id
        2.低延迟-急速响应
        3.高QPS-同时收到10万个id创建的请求，保证服务可用，不宕机
    5.分布式id生成的一般通用方案：
        1.uuid：-> 单机自用，不推荐上生产
            1.介绍：标准形式包含 32个16进制数字，以连字号分为5段，形式为 8-4-4-4-12的36个字符，例如 12qwweee-122w-122w-122w-qawsedasdqw1，一般会去掉连字号
            2.生成性能：本地生成，性能很高，无网络消耗
            3.使用性能：
                1.由于无序，入数据库性能差，违反btree索引，且mysql官方推荐主键id应越短越好，而uuid很长
                2.索引，B+树索引的分裂
                    分布式id作为主键，而主键是包含索引的，让后mysql的索引是通过B+tree实现的，每一次新的uuid插入，为了查询优化，都会对底层的B+tree进行修改
                    因为uuid数据时无序的，所以每一次uuid插入都会对现有b+tree进行很大的修改，不但会导致一些中间节点产生分裂，也不白白创建出很多不饱和的节点，
                    大大降低的数据库的插入性能
        2.数据库自增id
            1.单机-可以使用
                数据库自增id原理：自增id+数据库replace into实现的
                replace into：如果id已存在，则替换数据，否则插入
                    select last_insert_id()
            2.集群-不合适
                1.系统水平拓展比较困难，比如定义好步长和机器台数之后，如果要添加机器该怎么做？假设现在只有一台机器发号是1，2,3,4,5（步长为1），这个时候需要扩容一台机器。此时
                    可以把第二天机器的初始值设置的比第一台超过很多，貌似还好，现在想象以下如果线上有100台机器，此时扩容该怎么做，故难以实现
                2.数据库压力大，每次获取id都要读写一次数据库（select last_insert_id()），非常影响性能，不符合分布式id硬性条件 低延迟，高qps规则
                    （高并发下，都跑去数据库里获取id，是非常影响性能的）
        3.基于redis生成的全局id策略
            redis5.0 支持单线程 
            redis6.0 支持多线程
            1.单机版：因为redis是单线程的，天生保证原子性，可以使用原子操作incr和incrby来实现，符合所有分布式id的硬性规则
            2.集群版：在redis集群下，同样和mysql一样需要设置不同的增长步长，同时key一定要设置有效期，默认是永不过期
                1.可以使用redis获取更高的吞吐量
                2.假如一个集群有5台redis，可以初始化每台redis的值分别是1,2,3,4,5，然后步长都是5，那么各个redis生成的id为
                    A - 1,6,11,16,21
                    B - 2,7,12,17,22
                    C - 3,8,13,18,23
                    D - 4,9,14,19,24
                    E - 5,10,15,20,25
                3.所以一般小厂是可以使用redis集群做分布式id的，可落地
                4.缺点：配置麻烦，维护难，资源占用多-5台redis
                    若某个redis宕机可能导致号不连续的问题
        
### 21.2   （150） 雪花算法下

    1.snowflake：Twitter的分布式自增id算法snowflake，比redis又好用很多，可以按自己规则使用源码修改
    2.概述:
        twitter的雪花算法解决了全局唯一的分布式id问题，最初twitter把存储系统从mysql迁移到Cassandra（由facebook开发一套开源分布式nosql数据库系统），因为Casssandra没有顺序id生成机制，所以开发了这样
        一套全局唯一id生成服务。
    3.twitter的雪花算法经过测试每秒能产生26万个自增可排序的id
        1.生成的id能够按照时间有序生成
        2.生成的id是个64bit大小的整数，是一个Long型（转换成字符串后长度最多19）
        3.分布式系统内不会产生id碰撞（由datacenter和workid作区分），并且效率较高
    4.分布式，生成全局唯一id的基本要求
        1.分布式环境下必须全局且唯一
        2.一般都需要单调递增，因为一般唯一id都会存储到数据库，而innodb的特性就是将内容存储在主键索引树上的叶子节点上，而且是从左往右递增，所以考虑到数据库性能，一般生成的id最好是单调递增
            为了防止id冲突可以使用36位的uuid，但是uuid有一些缺点，长度太长且无序
    5.雪花算法的核心组成部分：java中是64bit的整数-long类型
        1bit符号位 +   41bit时间戳位   +   10bit工作进程位  +   12bit序列号位
        解析：
            1bit符号位：
                不用关心，因为二进制的最高位是符号位，1表示负数，0表示正数。生成的id一般都是用整数，所以最高位固定是0
            41bit时间戳位：
                极端情况下，41位都为1的时候，翻译成二进制
                2^41/(365*24*60*60*1000L)=69.73年    
                (365*24*60*60*1000L)毫秒
                系统可用时间69.73年，大概是从1970年到2039年
                -41位可以表示2^41-1个数字
                -如果只用来表示正整数，计算机中正数包含0，可以表示的数值范围是0至2^41-1,减1是因为可表示的数值范围从0开始算，而不是1
                -也就是41位可以表示2^41-1个毫秒的值，转换成单位年则是2^41/(365*24*60*60*1000L)=69.73年    
            10bit工作进程位：工作机器id
                -可以部署在2^10=1024个节点，包括5位datacenterid和5位workid
                -5位（bit）可以表示最大的正整数是2^5-1=31,即可以用0,1,2...31这32个数字，来表示不同的datacenterid或workid
                -表示现在可以有32个机房，每个机房有32台机器，一共是1024个节点
            12bit序列号，用来记录同毫秒内产生的不同id
                -12位bit可以表示的最大正整数是2^12-1=4095,即可以用0,1,2，...4094这4095个数字，来表示同一个机器，同一时间戳（毫秒）内产生的4095个id序列号
        雪花算法可以保证
            生成的id按时间趋势递增
            整个分布式系统内不会产生重复的id（因为有datacenterid和workid来做区分）
    6.源码：
    ...
    7.使用
        com.atguigu.springcloud.snowflake.SnowflakeIdWorker 中此类是用来学习的
        工作中使用：落地使用
        1.hutool工具包
        2.springboot整合雪花算法
            pom:
    ...
    8.优缺点：
        优点：
            ·毫秒数在高位，自增序列在低位，整个id都是趋势自增的
            ·不依赖数据库等第三方系统，以服务的方式部署，稳定性更高，生成id的性能也很高
            ·可以根据自身业务特性分配bit位，非常灵活
        缺点：    
            ·强依赖时钟，如果时钟回拨，会生成重复id
            ·在单机递增，若是涉及到分布式，每台机器上的时钟不可能完全同步，有时候会出现不是全局递增的情况
            （此缺点影响不大，一般分布式id只要求趋势递增，并不会严格要求递增，90%的需求都只要求趋势递增）
    9.补充：
